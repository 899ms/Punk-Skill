...
3. Confirm style before generating any prompt:
   ...
   - If no style is specified, recommend exactly three catalog styles based on the content and give a one-sentence reason for each, then ask the user to choose one or provide a custom style direction.
   - Do not show all 11 styles by default unless the user asks for the full menu.
...
4. Use this confirmation gate:
   - When platform/ratio or style is missing, stop after asking the question. Do not fill a template, save prompt files, or generate an image.
